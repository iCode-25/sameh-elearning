...
@section('title', \App\Helpers\TranslationHelper::translate('Gallerys'))
...
{{--                        @can('place_gallerys.create')--}}

 @if (auth()->guard('admin')->user()->can('gallery.create', 'admin'))
                            <a href="{{ route('admin.gallery.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Gallery')}}
                            </a>
                             @endif
{{--                        @endcan--}}
{{--                        @can('place_gallerys.sort')--}}

 @if (auth()->guard('admin')->user()->can('gallery.sort', 'admin'))
                            <a href="{{ route('admin.gallery.reorder') }}" class="btn btn-primary text-gray-800"
...
var table1 = $('#kt_table_users').DataTable({
    ...
    order: [[0, "desc"]],
    ajax: {
        url: '{{ route('admin.gallery.table') }}',
        data: function (d) {
            d.id = $('#id').val();
            d.name = $('#name').val();
            d.categorygallery_name = $('#categorygallery_name').val(); // إرسال اسم القسم
    ...
    columns: [
        ...
        // {data: 'name', name: 'name', defaultContent: '-', searchable: true},
        {data: 'category', name: 'category', defaultContent: '-', searchable: true}, // بحث باستخدام اسم القسم
