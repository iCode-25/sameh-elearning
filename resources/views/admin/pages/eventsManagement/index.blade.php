...
@section('title', \App\Helpers\TranslationHelper::translate('EventsManagement'))
...
{{--                        @can('place_eventsManagements.create')--}}

                              @if (auth()->guard('admin')->user()->can('eventsManagement.create', 'admin'))
                            <a href="{{ route('admin.eventsManagement.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create EventsManagement')}}
                            </a>
                            @endif
{{--                        @endcan--}}
{{--                        @can('place_eventsManagements.sort')--}}
                           @if (auth()->guard('admin')->user()->can('eventsManagement.sort', 'admin'))
                            <a href="{{ route('admin.eventsManagement.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    {{-- <script>
        ...
        var table1 = $('#kt_table_users').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: '{{ route('admin.eventsManagement.table') }}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
