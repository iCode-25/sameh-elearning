...
@if ($method == 'PUT')
    @section('title', \App\Helpers\TranslationHelper::translate('Edit EventsManagement'))
@else
    @section('title', \App\Helpers\TranslationHelper::translate('Add EventsManagement'))
@endif

@section('crumb')
    <x-bread-crumb :breadcrumbs="[
        ['text' => \App\Helpers\TranslationHelper::translate('EventsManagement'), 'link' => route('admin.eventsManagement.index')],
        ['text' => __('methods.' . getLastKeyRoute(request()->route()->getName()))],
    ]"
                   :button="['text' => \App\Helpers\TranslationHelper::translate('Go to EventsManagement'), 'link' => route('admin.eventsManagement.index')]">
    </x-bread-crumb>
@endsection
...
@section('content')
                    ...
                    <div class="card-header border-0 pt-6">
                        ...
                        <div class="card-body pt-0">
                            ...
                            <form action="{{ $action }}" method="POST" enctype="multipart/form-data">
                                ...
                                @if ($method == 'PUT')
                                    @method('PUT')
                                    <input type="hidden" name="id" value="{{ $eventsManagement->id }}">
                            @endif
                            <!--begin::Input group-->
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="col-6 mb-5">
                                                ...
                                                <input type="text" class="form-control form-control-solid"
                                                       value="{{ old('name.'.$key) ?? $eventsManagement->getTranslation('name',$key)}}"
                                        ...
                                        @endforeach
                                            ...
                                            <textarea class="form-control form-control-solid full-editor" name="description[{{ $key}}]" rows="5" id="description{{$key}}"
                                                      style="height: 300px; direction: rtl;">{{ old('description.'.$key) ?? $eventsManagement->getTranslation('description',$key)}}</textarea>
                                        ...
                                        @endforeach
                                         ...
                                         {{-- <div class="col-6 mb-5">
                                            ...
                                            <input type="text" class="form-control form-control-solid"
                                                   value="{{ old('name') ?? $eventsManagement->name }}"
    ...
    {{-- <div class="col-12 mb-5">
        ...
        <textarea class="form-control form-control-solid full-editor" name="description" rows="5" id="description"
                   style="height: 300px; direction: rtl;">{{ old('description') ?? $eventsManagement->description }}</textarea>
        @error('description')
            <span class="text-danger" role="alert">
                <strong>{{ $message }}</strong>
            </span>
...
{{-- <div class="col-12 mb-5">
    ...
    <textarea 
        ...
        {{ old('description') ?? $eventsManagement->description }}
    </textarea>
...
<div class="col-6 mb-5">
    <label class="fs-5 fw-bold form-label mb-5">
        {{ \App\Helpers\TranslationHelper::translate('Media image || video') }} :
        <span class="text-danger">
            {{ \App\Helpers\TranslationHelper::translate('Size') }} <span dir="ltr">400 x 400</span>
        </span>
    </label>
    <input type="file" class="form-control form-control-solid" name="media" accept="image/*, video/*"/>
    @error('media')
        <span class="text-danger" role="alert">
            <strong>{{ $message }}</strong>
        </span>
    @enderror
    @if ($eventsManagement->getFirstMediaUrl('events_management_image'))
        <img src="{{ $eventsManagement->getFirstMediaUrl('events_management_image') }}" alt="eventsManagements"
             width="100px" style="border-radius: 5px">
    @elseif ($eventsManagement->getFirstMediaUrl('events_managementvideo'))
        <video width="100px" controls>
            <source src="{{ $eventsManagement->getFirstMediaUrl('events_managementvideo') }}" type="video/mp4">
        </video>
    @endif
</div>
                                   ...
                                   {{-- <div class="col-6  mb-5">
                                            ...
                                            @enderror
                                            @if ($eventsManagement->getFirstMediaUrl('content_management_image') != null)
                                                <img src="{{ $eventsManagement->getFirstMediaUrl('content_management_image') }}" alt="eventsManagements"
                                                     width="100px" style="border-radius: 5px">
                                            @endif
                                        </div>
    ...
    @enderror
    @if ($eventsManagement->getFirstMediaUrl('pdf') != null)
        <a href="{{ $eventsManagement->getFirstMediaUrl('pdf') }}" target="_blank" class="btn btn-link">
 ...
 <div class="col-6 mb-5">
                ...
                @if($eventsManagement->getFirstMediaUrl('content_managementvideo'))
                    <div class="mt-4">
                        <h5>{{ \App\Helpers\TranslationHelper::translate('Existing Video') }}:</h5>
                        <video width="100%" controls>
                            <source src="{{ $eventsManagement->getFirstMediaUrl('content_managementvideo') }}" type="video/mp4">
                                        ...
                                        <div class="col-6  mb-5">
                                            ...
                                            <select class="form-select form-select-solid" name="category_id">
                                                ...
                                                @foreach ($categories as $category)
                                                    <option
                                                        value="{{ $category->id }}" {{ $eventsManagement->category_id == $category->id ? 'selected' : '' }}>
                                                        {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
...
{{-- 
                                        <div class="col-6  mb-5">
                                            ...
                                            @enderror
                                            @if ($eventsManagement->getFirstMediaUrl('eventsManagements_meta_image') != null)
                                                <img src="{{ $eventsManagement->getFirstMediaUrl('eventsManagements_meta_image') }}" alt="eventsManagements"
                                                     width="100px" style="border-radius: 5px">
                                            @endif
                                        </div>
                                        ...
                                        <div class="col-6  mb-5">
                                            ...
                                            <input type="text" class="form-control form-control-solid" name="meta_title"
                                                   value="{{ old('meta_title') ?? $eventsManagement->meta_title }}"/>
                                            @error('meta_title')
                                            <span class="text-danger" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
...
{{-- 
                                        <div class="col-6  mb-5">
                                            ...
                                            <input type="text" class="form-control form-control-solid" name="alt_text"
                                                   value="{{ old('alt_text') ?? $eventsManagement->alt_text }}"/>
                                            @error('alt_text')
                                            <span class="text-danger" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                        ...
                                        <div class="col-6  mb-5" id="-meta-tags">
                                            ...
                                            <select name="meta_tags[]" id="meta_tags" class="form-control" multiple>
                                                @if(isset($tags) && $tags->count() > 0)
                                                    @if($method == 'PUT')
                                                        @foreach($tags as $tag)
                                                            <option value="{{$tag->id}}" {{in_array($tag->id, $eventsManagement->tags()->pluck('tag_id')->toArray()) ? 'selected' : ''}}>{{$tag->name}}</option>
                                                        @endforeach
                                                    @else
                                                        @foreach($tags as $tag)
                                        ...
                                        {{-- <div class="col-6  mb-5">
                                            ...
                                            <textarea class="form-control form-control-solid" name="meta_description"
                                                      rows="5"
                                                      style="">{{ old('meta_description') ?? $eventsManagement->meta_description }}</textarea>
