...
{{--                        @can('place_categories.create')--}}

                            <a href="{{ route('admin.categorygallery.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Categorygallery')}}
                            </a>
{{--                        @endcan--}}
{{--                        @can('place_categories.sort')--}}

                            <a href="{{ route('admin.categorygallery.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    {{-- <script>
        ...
        var table1 = $('#kt_table_users').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: '{{ route('admin.categorygallery.table') }}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
...
var table1 = $('#kt_table_users').DataTable({
    ...
    columns: [
        {data: 'id', name: 'id', defaultContent: '-'},
        {data: 'image', name: 'image', defaultContent: '-', orderable: false, searchable: false},
        {data: 'name', name: 'name', defaultContent: '-', searchable: true}, // البحث بالاسم
        {data: 'created_at', name: 'created_at', defaultContent: '-'},
        {data: 'action', orderable: false, searchable: false, className: 'text-center'}
    ],
    createdRow: function (row, data, index) {
