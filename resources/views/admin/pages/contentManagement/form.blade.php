...
@if ($method == 'PUT')
    @section('title', \App\Helpers\TranslationHelper::translate('Edit Teachers And parents Content'))
@else
    @section('title', \App\Helpers\TranslationHelper::translate('Add Teachers And parents Content'))
@endif

@section('crumb')
    <x-bread-crumb :breadcrumbs="[
        [
            'text' => \App\Helpers\TranslationHelper::translate('Teachers And parents Content'),
            'link' => route('admin.contentManagement.index'),
        ],
        ['text' => __('methods.' . getLastKeyRoute(request()->route()->getName()))],
    ]" :button="[
        'text' => \App\Helpers\TranslationHelper::translate('Go to Teachers And parents Content'),
        'link' => route('admin.contentManagement.index'),
    ]">
    </x-bread-crumb>
@endsection
                                ...
                                <div class="fv-row mb-10">
                                    <div class="row">
                                        @foreach (Config('language') as $key => $lang)
                                            <div class="col-6 mb-5">
                                                ...
                                                <input type="text" class="form-control form-control-solid"
                                                    value="{{ old('name.' . $key) ?? $contentManagement->getTranslation('name', $key) }}"
                                        ...
                                        @foreach (Config('language') as $key => $lang)
                                            <div class="col-12  mb-5">
                                                ...
                                                <textarea class="form-control form-control-solid full-editor" name="description[{{ $key }}]" rows="5"
                                                    id="description{{ $key }}" style="height: 300px; direction: rtl;">{{ old('description.' . $key) ?? $contentManagement->getTranslation('description', $key) }}</textarea>
                                        ...
                                        <div class="col-6  mb-5">
                                            ...
                                            @enderror
                                            @if ($contentManagement->getFirstMediaUrl('content_management_image') != null)
                                                <img src="{{ $contentManagement->getFirstMediaUrl('content_management_image') }}"
                                                    alt="contentManagements" width="100px" style="border-radius: 5px">
                                            @endif
                                        </div>
                                        <div class="col-6 mb-5">
                                            <label class="fs-5 fw-bold form-label mb-5">
                                                {{ \App\Helpers\TranslationHelper::translate('Upload File (PDF or Video)') }}
                                                :
                                                <span class="text-danger">
                                                    {{ \App\Helpers\TranslationHelper::translate('Allowed Formats') }}:
                                                    PDF, Video
                                                </span>
                                            </label>
                                            ...
                                            @if ($contentManagement->getFirstMediaUrl('pdf'))
                                                <a href="{{ $contentManagement->getFirstMediaUrl('pdf') }}" target="_blank"
                                                    class="btn btn-link">
                                                    {{ \App\Helpers\TranslationHelper::translate('View Uploaded PDF') }}
                                                </a>
                                            @elseif ($contentManagement->getFirstMediaUrl('content_managementvideo'))
                                                <div class="mt-4">
