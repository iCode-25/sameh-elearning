...
@section('title', \App\Helpers\TranslationHelper::translate('Teachers And parents Content'))

@section('crumb')
    <x-bread-crumb :breadcrumbs="[
        [
            'text' => \App\Helpers\TranslationHelper::translate('Content'),
            ...
            <div class="card-toolbar w-100 mb-7">
                ...
                <div class="d-flex justify-content-between w-100 data-kt-Customer-table-toolbar">
                    <div class="left-part">
                        ...
                        {{--                        @can('place_contentManagements.create') --}}

                        @if (auth()->guard('admin')->user()->can('contentManagement.create', 'admin'))
                            <a href="{{ route('admin.contentManagement.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{ \App\Helpers\TranslationHelper::translate('Create Teachers And parents Content') }}
                            </a>
                        @endif
                        {{--                        @endcan --}}
                        {{--                        @can('place_contentManagements.sort') --}}
                        @if (auth()->guard('admin')->user()->can('contentManagement.sort', 'admin'))
                            <a href="{{ route('admin.contentManagement.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    <script>
        var table1 = $('#kt_table_users').DataTable({
            ...
            ajax: {
                url: '{{ route('admin.contentManagement.table') }}',
                data: function(d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
