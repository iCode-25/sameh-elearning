{{--@can('place_complaints.view_details')--}}
   @if (auth()->guard('admin')->user()->can('complaint.view_details', 'admin'))
<a href="{{route('admin.complaint.show',['complaint'=>$complaint->id])}}"
   class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{\App\Helpers\TranslationHelper::translate('Show Details')}}">
    <img src="{{asset('dashboard/assets/img/show.png')}}" alt="show" style="width: 25px">
</a>
@endif
{{--@endcan--}}
{{--@can('place_complaints.edit')--}}
   {{-- @if (auth()->guard('admin')->user()->can('complaint.edit', 'admin'))
<a href="{{route('admin.complaint.edit',['complaint'=>$complaint->id])}}"
   class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{\App\Helpers\TranslationHelper::translate('Edit')}}">
    <img src="{{asset('dashboard/assets/img/edit.png')}}" alt="edit" style="width: 25px">
</a>
@endif --}}
{{--@endcan--}}

{{--@can('place_complaints.delete')--}}

   @if (auth()->guard('admin')->user()->can('complaint.delete', 'admin'))
<button type="button" data-url="{{route('admin.complaint.destroy',['complaint'=>$complaint->id])}}"
   data-item-id="{{ $complaint->id }}"
   class="btn btn-sm btn-clean btn-icon btn-icon-md delete-button"
   data-toggle="modal"
   data-target="#delete_modal">
    <img src="{{asset('dashboard/assets/img/delete.png')}}" alt="edit" style="width: 25px">
</button>
@endif

{{--@endcan--}}

<script src="{{ asset('dashboard/assets/js/delete-item.js') }}" type="text/javascript"></script>
