...
@section('title', \App\Helpers\TranslationHelper::translate('Complaints'))

@section('crumb')
    <x-bread-crumb :breadcrumbs="[
    ['text'=> \App\Helpers\TranslationHelper::translate('Complaints'),'link'=>route('admin.complaint.index')],
...
{{--                        @can('place_complaints.create')--}}

                      {{-- @if (auth()->guard('admin')->user()->can('complaint.create', 'admin'))
                            <a href="{{ route('admin.complaint.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Complaint')}}
                            </a>
                            @endif --}}

{{--                        @endcan--}}

{{--                        @can('place_complaints.sort')--}}

                            {{-- <a href="{{ route('admin.complaint.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    <script>
        ...
        var table1 = $('#kt_table_users').DataTable({
            ...
            // order: [[0, "desc"]],
            ajax: {
                url: '{{ route('admin.complaint.table') }}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
