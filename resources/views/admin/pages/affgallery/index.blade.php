...
@section('title', \App\Helpers\TranslationHelper::translate('galleries'))

@section('crumb')
    <x-bread-crumb :breadcrumbs="[
    ['text'=> \App\Helpers\TranslationHelper::translate('galleries'),'link'=>route('admin.aff_gallery.index',  $gallery_id)],
...
{{--  --}}
                            <a href="{{route('admin.aff_gallery.create',['gallery_id'=> $gallery_id])}}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create add_Gallery')}}
                            </a>

{{--                        @endcan--}}
{{--                        @can('place_gallerys.sort')--}}

                            {{-- <a href="{{ route('admin.aff_gallery.reorder') }}" class="btn btn-primary text-gray-800"
...
{{--                        @endcan--}}
            ...
            <table class="table table-rounded align-middle table-striped border gy-5 gs-7 dataTable no-footer"
                ...
                <tr>
                    <th>#</th>
                    <th>{{\App\Helpers\TranslationHelper::translate('add_Image')}}</th>
...
@push('admin_js')
    ...
    <script>
        ...
        var table1 = $('#kt_table_users').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: '{{ route('admin.aff_gallery.table', ['gallery_id' => $gallery_id]) }}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
