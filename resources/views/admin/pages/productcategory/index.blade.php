...
@section('title', \App\Helpers\TranslationHelper::translate('Product categorys'))

@section('crumb')
    <x-bread-crumb :breadcrumbs="[
    ['text'=> \App\Helpers\TranslationHelper::translate('Product categorys'),'link'=>route('admin.productcategory.index')],
                ...
                <div class="d-flex justify-content-between w-100 data-kt-Customer-table-toolbar">
                    <div class="left-part">
                        <!--begin::Add Service-->
                           ...
                           @if (auth()->guard('admin')->user()->can('productcategory.create', 'admin'))
                            <a href="{{ route('admin.productcategory.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Product category')}}
                            </a>
                            @endif


{{--                        @endcan--}}
{{--                        @can('place_productcategorys.sort')--}}

                            {{-- <a href="{{ route('admin.productcategory.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    order: [[0, "desc"]],
    ajax: {
        url: '{{ route('admin.productcategory.table') }}',
        data: function (d) {
            d.id = $('#id').val();
            d.name = $('#name').val(); 
    ...
    columns: [
        {data: 'id', name: 'id', defaultContent: '-', searchable: false},
        //  {data: 'image', name: 'image', defaultContent: '-', searchable: false},
        //  {data: 'image', name: 'image', defaultContent: '-', orderable: false, searchable: false},
        {data: 'name', name: 'name', defaultContent: '-', searchable: false},
