...
@section('title', \App\Helpers\TranslationHelper::translate('Challenges'))
...
{{--                        @can('place_challengess.create')--}}

                              @if (auth()->guard('admin')->user()->can('challenges.create', 'admin'))
                            <a href="{{ route('admin.challenges.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Challenge')}}
                            </a>
                            @endif
{{--                        @endcan--}}
{{--                        @can('place_challengess.sort')--}}
                           @if (auth()->guard('admin')->user()->can('challenges.sort', 'admin'))
                            <a href="{{ route('admin.challenges.reorder') }}" class="btn btn-primary text-gray-800"
...
{{--                        @endcan--}}
            ...
            <table class="table table-rounded align-middle table-striped border gy-5 gs-7 dataTable no-footer"
                ...
                <tr>
                    ...
                    {{-- <th>{{\App\Helpers\TranslationHelper::translate('category')}}</th> --}}
...
@push('admin_js')
    ...
    {{-- <script>
        ...
        var table1 = $('#kt_table_users').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: '{{ route('admin.challenges.table') }}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
    ...
    ajax: {
        ...
        data: function (d) {
            d.id = $('#id').val();
            d.name = $('#name').val();
            d.category = $('#category').val(); // البحث عن القسم
            d.created_at = $('#created_at').val();
        }
    },
    columns: [
...
$(document).on('change', '.active-toggle', function () {
    ...
    $.ajax({
        url: '{{ route("admin.challenges.toggleActiveStatus") }}', // المسار الذي سيستقبل الطلب
        type: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
            id: clientId,
        ...
        success: function (response) {
            if (response.success) {
                Swal.fire(
                    'Success!',
                    response.is_active ? 'Challenge is activated.' : 'Challenge is deactivated.',
                    'success'
                );
            } else {
                Swal.fire('Error!', 'Failed to update Challenge status.', 'error');
            }
        },
        error: function () {
