...
@if ($method == 'PUT')
    @section('title', \App\Helpers\TranslationHelper::translate('Edit Challenges'))
@else
    @section('title', \App\Helpers\TranslationHelper::translate('Add Challenges'))
@endif
...
@section('crumb')
    <x-bread-crumb :breadcrumbs="[
        ...
        ['text' => __('methods.' . getLastKeyRoute(request()->route()->getName()))],
                   ...
                   :button="['text' => \App\Helpers\TranslationHelper::translate('Go to Challenges'), 'link' => route('admin.challenges.index')]">
    </x-bread-crumb>
@endsection
...
@section('content')
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="col-6 mb-5">
                                                ...
                                                <input type="text" class="form-control form-control-solid"
                                                       value="{{ old('name.'.$key) ?? $challenges->getTranslation('name',$key)}}"
                                        ...
                                        @endforeach
                                            ...
                                            <textarea class="form-control form-control-solid full-editor" name="description[{{ $key}}]" rows="5" id="description{{$key}}"
                                                      style="height: 300px; direction: rtl;">{{ old('description.'.$key) ?? $challenges->getTranslation('description',$key)}}</textarea>
    ...
    <input type="date" class="form-control form-control-solid"
           value="{{ old('date') ?? $challenges->date }}"
           name="date"/>
    ...
    @enderror
                                         ...
                                         {{-- <div class="col-6 mb-5">
                                            ...
                                            <input type="text" class="form-control form-control-solid"
                                                   value="{{ old('name') ?? $challenges->name }}"
    ...
    {{-- <div class="col-12 mb-5">
        ...
        <textarea class="form-control form-control-solid full-editor" name="description" rows="5" id="description"
                   style="height: 300px; direction: rtl;">{{ old('description') ?? $challenges->description }}</textarea>
        @error('description')
            <span class="text-danger" role="alert">
                <strong>{{ $message }}</strong>
            </span>
...
{{-- <div class="col-12 mb-5">
    ...
    <textarea
        ...
        {{ old('description') ?? $challenges->description }}
    </textarea>
    ...
    @enderror
                                   ...
                                   <div class="col-6  mb-5">
                                            ...
                                            @enderror
                                            @if ($challenges->getFirstMediaUrl('challenges_image') != null)
                                                <img src="{{ $challenges->getFirstMediaUrl('challenges_image') }}" alt="challengess"
    ...
    @enderror
    @if ($challenges->getFirstMediaUrl('pdf') != null)
        <a href="{{ $challenges->getFirstMediaUrl('pdf') }}" target="_blank" class="btn btn-link">
                                        ...
                                        <div class="col-6  mb-5">
                                            ...
                                            <select class="form-select form-select-solid" name="category_id">
                                                ...
                                                @foreach ($categories as $category)
                                                    <option
                                                        value="{{ $category->id }}" {{ $challenges->category_id == $category->id ? 'selected' : '' }}>
                                                        {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
...
{{--
                                        <div class="col-6  mb-5">
                                            ...
                                            @enderror
                                            @if ($challenges->getFirstMediaUrl('challengess_meta_image') != null)
                                                <img src="{{ $challenges->getFirstMediaUrl('challengess_meta_image') }}" alt="challengess"
                                                     width="100px" style="border-radius: 5px">
                                            @endif
                                        </div>
                                        ...
                                        <div class="col-6  mb-5">
                                            ...
                                            <input type="text" class="form-control form-control-solid" name="meta_title"
                                                   value="{{ old('meta_title') ?? $challenges->meta_title }}"/>
                                            @error('meta_title')
                                            <span class="text-danger" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
...
{{--
                                        <div class="col-6  mb-5">
                                            ...
                                            <input type="text" class="form-control form-control-solid" name="alt_text"
                                                   value="{{ old('alt_text') ?? $challenges->alt_text }}"/>
                                            @error('alt_text')
                                            <span class="text-danger" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                        ...
                                        <div class="col-6  mb-5" id="-meta-tags">
                                            ...
                                            <select name="meta_tags[]" id="meta_tags" class="form-control" multiple>
                                                @if(isset($tags) && $tags->count() > 0)
                                                    @if($method == 'PUT')
                                                        @foreach($tags as $tag)
                                                            <option value="{{$tag->id}}" {{in_array($tag->id, $challenges->tags()->pluck('tag_id')->toArray()) ? 'selected' : ''}}>{{$tag->name}}</option>
                                                        @endforeach
                                                    @else
                                                        @foreach($tags as $tag)
                                        ...
                                        {{-- <div class="col-6  mb-5">
                                            ...
                                            <textarea class="form-control form-control-solid" name="meta_description"
                                                      rows="5"
                                                      style="">{{ old('meta_description') ?? $challenges->meta_description }}</textarea>
