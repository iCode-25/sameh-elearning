...
@section('title', \App\Helpers\TranslationHelper::translate('Paintscategorys'))

@section('crumb')
    <x-bread-crumb :breadcrumbs="[
    ['text'=> \App\Helpers\TranslationHelper::translate('Paintscategorys'),'link'=>route('admin.paintscategory.index')],
                    ...
                    <div class="left-part">
                        <!--begin::Add Service-->
                           ...
                           @if (auth()->guard('admin')->user()->can('paintscategory.create', 'admin'))
                            <a href="{{ route('admin.paintscategory.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Paintscategory')}}
                            </a>
                            @endif


{{--                        @endcan--}}
{{--                        @can('place_paintscategorys.sort')--}}

                            {{-- <a href="{{ route('admin.paintscategory.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    order: [[0, "desc"]],
    ajax: {
        url: '{{ route('admin.paintscategory.table') }}',
        data: function (d) {
            d.id = $('#id').val();
            d.name = $('#name').val(); 
