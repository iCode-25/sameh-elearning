...
@section('title', \App\Helpers\TranslationHelper::translate('Countries'))
...
{{--                        @can('place_placecats.create')--}}

                            <a href="{{ route('admin.placecat.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Countries')}}
                            </a>
{{--                        @endcan--}}
{{--                        @can('place_placecats.sort')--}}

                            <a href="{{ route('admin.placecat.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    <script>
        ...
        var table1 = $('#kt_table_users').DataTable({
            ...
            order: [[0, "desc"]],
            ajax: {
                url: '{{ route('admin.placecat.table') }}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
