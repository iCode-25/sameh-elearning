...
@section('title', \App\Helpers\TranslationHelper::translate('Notifications'))
...
{{--                        @can('place_notifications.create')--}}

                            {{-- <a href="{{ route('admin.notification.create') }}" class="btn btn-primary text-gray-800">
                                <i class="ki-duotone ki-plus fs-2  text-gray-800"></i>
                                {{\App\Helpers\TranslationHelper::translate('Create Notification')}}
                            </a> --}}


{{--                        @endcan--}}
{{--                        @can('place_notifications.sort')--}}

                            {{-- <a href="{{ route('admin.notification.reorder') }}" class="btn btn-primary text-gray-800"
...
@push('admin_js')
    ...
    <script>
        ...
        var table1 = $('#kt_table_users').DataTable({
            ...
            // order: [[0, "desc"]],
            ajax: {
                url: '{{ route('admin.notification.table') }}',
                data: function (d) {
                    d.id = $('#id').val();
                    d.name = $('#name').val();
