...
@if($method == 'PUT')
    @section('title', \App\Helpers\TranslationHelper::translate('Edit Country'))
@else
    @section('title', \App\Helpers\TranslationHelper::translate('Add Country'))
@endif
...
@section('crumb')
    ...
    ]" :button="['text'=>\App\Helpers\TranslationHelper::translate('Go to Country'),'link'=>route('admin.country.index')]">
    </x-bread-crumb>
@endsection
                                        ...
                                        @foreach (Config('language') as $key => $lang)
                                            <div class="col-6 mb-5">
                                                ...
                                                <input type="text" class="form-control form-control-solid"
                                                       value="{{ old('name.'.$key) ?? $country->getTranslation('name',$key)}}"
                                                       placeholder="{{\App\Helpers\TranslationHelper::translate('Name in')}} {{__('methods.' . $lang)}}" name="name[{{ $key}}]"/>
                                                @error('name.'.$key)
...
{{-- 
                                            ...
                                            <div class="col-6 mb-5">
                                                ...
                                                <!--begin::Input-->
                                                <input type="text" class="form-control form-control-solid"
                                                       value="{{ old('meta_title.'.$key) ?? $country->getTranslation('meta_title',$key)}}"
                                                       placeholder="{{\App\Helpers\TranslationHelper::translate('meta_title in')}} {{__('methods.' . $lang)}}" name="meta_title[{{ $key}}]"/>
                                        ...
                                        @endforeach --}}
                                            ...
                                            {{-- <div class="col-6  mb-5">
                                                ...
                                                @enderror
                                                @if($country->getFirstMediaUrl('countries') != null)
                                                    <img src="{{ $country->getFirstMediaUrl('countries') }}"
                                                         alt="countries" width="100px"
