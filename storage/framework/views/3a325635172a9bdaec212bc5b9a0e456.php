                            ...
                            <div class="col-lg-6 col-sm-6">
                                <div class="title-content">
                                    <h2 class="fw-bold mb-3" style="color: var(--primary-color);">
                                        <?php echo e($package->getTranslation('name', app()->getLocale())); ?></h2>
                                    <div class="mb-3">
                                        <span class="badge"
                                            style="background: var(--primary-color); color: #fff; font-size: 1rem;"><?php echo e($package->level->getTranslation('name', app()->getLocale())); ?></span>
                                    </div>
                                </div>
                                <div class="img-block">
                        ...
                        <div style="position: relative; width: 100%; max-width: 800px; margin: auto;">
                            ...
                            <div id="watermark"
                                style="position: absolute;
                                    ...
                                    z-index: 10;
                                ">
                                <?php echo e(auth('web')->user()->email); ?> - <?php echo e(now()->format('H:i:s')); ?>
...
<?php endif; ?>
                ...
                <div class="col-lg-4 col-md-12 px-2">
                    ...
                    <div class="card border-0 shadow rounded-4 p-2 bg-white mb-2">
                        ...
                        <?php if(isset($lesson->tests) && $lesson->tests()->active()->where('type', auth('web')->user()->type)->count() > 0): ?>
                    ...
                    <div class="card border-0 shadow rounded-4 p-2 bg-white mb-2">
                        <h4 class="fw-bold mb-3" style="color: var(--primary-color);">
                            <?php echo e(\App\Helpers\TranslationHelper::translate('package tests')); ?></h4>
                        <?php if(isset($package->tests) && $package->tests()->active()->where('type', auth('web')->user()->type)->count() > 0): ?>
                            <ul class="list-group list-group-flush">
                                <?php $__currentLoopData = $package->tests()->active()->where('type', auth('web')->user()->type)->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $test): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li class="list-group-item d-flex justify-content-between align-items-center mb-0 pb-0">
                                        <div
                                            title="<?php echo e($test->quizzes->count() == 0 ? \App\Helpers\TranslationHelper::translate('Quizzes not found') : \App\Helpers\TranslationHelper::translate('start')); ?>">
                                            <span class="fw-bold"
                                                style="color: var(--primary-color);"><?php echo e($test->name); ?></span>
                                            <span class="text-muted ms-2"><?php echo Str::limit($test->description, 40); ?></span>
                                        </div>
                                        <a href="<?php echo e(route('user.package.test.show', ['package' => $package->id, 'test' => $test->id])); ?>"
                                            class="btn btn-sm <?php echo e($test->quizzes->count() == 0 ? 'disabled' : ''); ?>"
                                            style="background: var(--primary-color); color: #fff; border-radius: 6px;"><?php echo e(\App\Helpers\TranslationHelper::translate('start test')); ?></a>
                                    </li>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </ul>
                        ...
                        <div class="card border-0 shadow rounded-4 p-2 bg-white">
                            ...
                            <div class="list-group list-group-flush px-2">
                                <?php $__empty_1 = true; $__currentLoopData = $package->lessons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $next): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <div onclick="window.location.href='<?php echo e(route('user.show_package', ['package' => $package->id, 'lesson' => $next->id])); ?>'"
...
<?php echo $__env->make('front.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\sameh-elearning\resources\views/front/pages/Packages/show_package.blade.php ENDPATH**/ ?>
